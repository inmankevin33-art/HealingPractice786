          ...
          >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
              ...
              <div className="mb-12 border-b border-slate-50 pb-12">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] mb-6">Select Your Location</p>
                <div className="flex flex-col sm:flex-row gap-4 max-w-xl">
                  <Link href="/" onClick={() => setIsMenuOpen(false)} className={`flex-1 flex items-center justify-center p-4 rounded-xl border-2 transition-all ${!isBirmingham ? 'border-blue-600 bg-blue-50/50 text-blue-900 font-bold' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}>
                    St Albans Clinic
                  </Link>
                  <Link href="/birmingham" onClick={() => setIsMenuOpen(false)} className={`flex-1 flex items-center justify-center p-4 rounded-xl border-2 transition-all ${isBirmingham ? 'border-blue-600 bg-blue-50/50 text-blue-900 font-bold' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}>
                    Birmingham Clinic
                  </Link>
                </div>
              </div>
              ...
              <nav className="flex flex-col space-y-4 md:space-y-6">
                {menuItems.map((item, idx) => (
                  ...
                  >
                    <Link 
                      href={item.href} 
                      className="text-2xl md:text-4xl font-raleway font-medium text-slate-900 hover:text-blue-600 transition-colors inline-block"
